<template>
  <div :class="{ 'upload-container': true, 'upload-error': explainErrorVisible }">
    <div class="upload-button-container" v-if="!disabled">
      <component
      ...
      <div class="upload-error-explain-error">{{ explainErrorText }}</div>
    </div>
    <!-- 组件内部自定义 -->
    <div class="custom-upload-list" v-if="isUploadItemRender">
      <div class="upload-item" v-for="(file, index) in fileList" :key="index">
        <div class="upload-item-left" v-if="pictureVisible && !isNull(file?.fileUrl)">
          <div class="upload-item-picture">
            <a-image :width="84" :src="file?.fileUrl" style="pointer-events: all" />
          </div>
        </div>
...
import { isNull, filterEnum, isBoolean, blobDownloadFile } from '@/utils/utils';
...
import { checkUploadTask, addShardTask, uploadShard, mergeShard, fileDelete, uploadFile } from '@/api/file';

const props = defineProps({
  listType: {
    type: String,
    default: 'file' // 展示方式：file/picture，picture需要预览
  },
  errorTip: {
    type: String,
    default: 'message' // 校验提示：border/message
  },
  trigger: {
    type: String,
    default: 'click' //上传方式：click/dragger
  },
  multiple: {
    type: Boolean,
    default: false
  ...
  shardSize: {
    type: Number,
    default: 200 * 1024 // FIXME:最小必须是5M ，分片大小，单位字节(KB)
  },
  accept: {
    type: String,
    default: null // 文件类型
  ...
  defaultFileList: {
    type: Array, // 回显文件列表 [{fileUrl: '', name: '', size: '', fileId: ''}]
    default: []
  },
  name: {
    type: String,
  ...
  uploadRequest: {
    type: [Function, Promise], // 不分片上传请求
    default: null
  },
  uploadParams: {
    type: Object, // 上传所需额外参数
    default: null
  },
  uploadedCallBack: {
    type: Function,
...
const isMessageErrorTip = computed(() => props.errorTip === 'message'); // 错误信息是否已message弹窗展示
const explainErrorText = ref('');
const explainErrorVisible = computed(() => !isMessageErrorTip.value && !isNull(explainErrorText.value));
const isDefaultButton = !useSlots().uploadButton; // 是否自定义点击按钮
const isUploadItemRender = !useSlots().uploadItemRender; // 是否自定义上传列表
const pictureVisible = computed(() => props.listType === 'picture');
// 终止请求
...
// 是否显示删除
const setDeleteVisible = (file) => {
  return props.maxCount !== 1 && file?.status !== UPLOAD_STATUS_UPLOADING;
};

// 是否显示下载
const setDownloadVisible = (file) => {
  return ![UPLOAD_STATUS_ERROR, UPLOAD_STATUS_UPLOADING, UPLOAD_STATUS_STOP]?.includes(file?.status);
};
...
const DraggerDom = (
  <>
    ...
    <p class='ant-upload-hint'>{`文件大小不能超过${props.size}M，仅支持 ${!isNull(props.accept) ? props.accept : '所有(*.*)'} 类型文件上传。${
      !isNull(props.maxCount) ? `最多只能上传${props.maxCount}个文件` : ''
    }`}</p>
  </>
);
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (!isNull(props.size) && !(file.size / 1024 / 1024 <= props.size)) {
        errorText = `注意：上传的文件不能大于${props.size}M!`;
        explainErrorText.value = errorText;
        isMessageErrorTip.value && message.error(errorText);
        return false;
      }

      // 文件数量，当maxCount是1时新文件覆盖旧文件，当maxCount>1时，已上传的文件超过maxCount时不能再上传
      if (!isNull(props.maxCount) && fileCount.value + fileList.length > props.maxCount && props.maxCount !== 1) {
        errorText = `注意：最多只能上传${props.maxCount}个文件！您选中的文件和已上传文件的数量超过限制了~（已上传${fileCount.value},当前选中${fileList.length}）`;
        explainErrorText.value = errorText;
        isMessageErrorTip.value && message.error(errorText);
        return false;
      }
...
const uploadChange = async (file) => {
  ...
  updateFileList({
    ...
    percent: 0,
    fileUrl: pictureVisible ? window.URL.createObjectURL(file.file) : null
  });
  upload(file.file);
};
...
const handleDownload = async (fileId) => {
  try {
    if (props.downloadRequest) {
      const filestream = await props.downloadRequest({ fileId });
      await blobDownloadFile(filestream?.file, filestream?.fileName);
    }
  } catch (error) {
...
const fileUpload = (options) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const API = props.uploadRequest || uploadFile;
      ...
      const fileId = result?.data?.fileId ? result?.data?.fileId : result?.data;
      updateFileList({ ...options, ...result?.data, fileId, status: UPLOAD_STATUS_SUCCESS, percent: percent || 1 });
    } catch (error) {
      reject(error);
      updateFileList({ ...options, status: UPLOAD_STATUS_ERROR, fileUrl: null });
    ...
    try {
      for (let i = 0; i < shardArr?.length; i++) {
        ...
        if (uploadedArr?.length === count) {
          ...
          const fileId = fileInfo?.data?.fileId ? fileInfo?.data?.fileId : fileInfo?.data;
          updateFileList({ ...options, ...fileInfo?.data, fileId, percent: 1, size, status: UPLOAD_STATUS_SUCCESS });
        }
      }
    } catch (error) {
      console.error('components-upload-shardUpload', error);
      reject(error);
      updateFileList({ ...options, percent: uploadedArr?.length / count, size, status: UPLOAD_STATUS_ERROR, fileUrl: null });
    }
  });
};
...
const updateFileList = (options) => {
  try {
    ...
    const index = _.findIndex(fileList.value, (item) => {
      if (!isNull(item?.fileId) && !isNull(fileId)) {
        return item?.fileId === fileId;
      }
      return item?.uid === uid || item?.file?.uid === uid;
    });
    if (index !== -1) {
      const oldVal = fileList.value[index];
      const newVal = {
        ...oldVal,
        ...options,
        percent: percent?.toFixed(2) * 100
      };
      fileList.value.splice(index, 1, newVal);
      // FIXME:给组件调用方回传状态
      props.uploadedCallBack && props.uploadedCallBack({ ...newVal });
    }
  } catch (error) {
...
const handleRemove = async (event, options) => {
  const { fileId, uid, status, fileName, name } = options || {};
  deleteRequestSource = axios.CancelToken.source();
  Modal.delete({
    title: '文件删除',
    content: `${status === UPLOAD_STATUS_UPLOADING ? '当前文件正在上传中，是否终止并删除？' : `确认要删除“${fileName || name}”文件吗？`}`,
    async onCancel() {
      try {
        ...
        if (!isNull(fileId)) {
          ...
          await deleteAPI({ ...props.uploadParams, fileId });
        }
  ...
  .custom-upload-list {
    ...
    .upload-item {
      ...
      .upload-item-left {
        ...
        //   border-radius: 8px;
        // }
        // .upload-item-picture:hover:before {
        //   opacity: 1;
        // }
      }
      .upload-item-center {
