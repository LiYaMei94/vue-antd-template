<template>
  ...
  >
    ...
    <a-form-item name="icon">
      <template #label>
        <QuestionTooltip v-bind="{ title: formItemTip?.icon, label: '菜单图标' }" :show-count="!isDetail" :maxlength="50"></QuestionTooltip>
      </template>
      <a-input placeholder="请输入菜单图标。示例：【icon-question-circle或IconQuestionCircle】" v-model:value="formState.icon"></a-input>
    </a-form-item>
    ...
    <a-form-item name="redirectPath" v-if="formState.isFrame !== CONST_STRING_1">
      <template #label>
        <QuestionTooltip v-bind="{ title: formItemTip?.redirectPath, label: '重定向路由' }"></QuestionTooltip>
      </template>
      <a-input placeholder="请输入重定向路由" v-model:value="formState.redirectPath" :show-count="!isDetail" :maxlength="200"></a-input>
    </a-form-item>
...
import { topNavRules } from '../js/editRules';
import { YES_OR_NO, CONST_STRING_1 } from '@/utils/const';
...
const handleSubmit = () => {
  try {
    ...
    console.error('menu-TopNav-handleSubmit', error);
...
const handleValidate = async () => {
  try {
    ...
    console.error('menu-TopNav-handleValidate', error);
    return Promise.reject(error);
  }
};
